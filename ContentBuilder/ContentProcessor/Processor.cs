...
@"// ------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by the Content Processor.
//
...
{{
    ...
    public readonly record struct FontID(int ID);
			...
			var classPath = Path.Combine(classOutputDir.FullName, "Fonts.g.cs");
...
{{
	...
	{{
		...
		{{
			...
			foreach (var page in TexturePage.IDLookup)
			{{
				if (maybeExtension == "".json"")
				{{
					if (partialPath == page.PartialJsonFilePath)
					{{
						return CramAtlasReader.ReadTextureAtlas(
							GraphicsDevice, 
							page, 
							storage
						);
...
{{
	...
	{{
		...
		{string.Join("\n\t\t", definitionStrings)}
			...
			var classPath = Path.Combine(classOutputDir.FullName, "Levels.g.cs");
			File.WriteAllText(classPath, textureAtlasesClassCode);
		}
...
";
			...
			classOutputDir.Create();
			var classPath = Path.Combine(classOutputDir.FullName, "Videos.g.cs");
